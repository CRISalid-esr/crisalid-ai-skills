...
async def test_restricted_toolset_tools(toolbox_client):
    ...
    assert names == {"get-crisalid-schema", "list-person-publications", "search-person-by-name"}
...
async def test_unrestricted_toolset_tools(toolbox_client):
    ...
    names = set(t.name for t in tools)
    assert names == {"get-crisalid-schema", "list-person-publications", "search-person-by-name", "execute-cypher-readonly"}
